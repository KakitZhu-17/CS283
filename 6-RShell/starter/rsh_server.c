...
int exec_client_requests(int cli_socket) {
    ...
    char *io_buff;
    int ret;
    int rcFirstState = 0; //acts like a boolean to make sure rc is a zero if we did not do a command before
...
int send_message_string(int cli_socket, char *buff){
    ...
    int sendMessage = send(cli_socket,buff, strlen(buff)+1,0);
    if(sendMessage == -1 ){
        perror("send: ");
        return ERR_RDSH_COMMUNICATION;
    }
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (pids[i] == 0) {  // Child process
            ...
            if(i == 0){
                int stdin = dup2(cli_sock, STDIN_FILENO);
                if(stdin == -1){
                    perror("stdin failed: ");
                    exit(-1);                
                }
            ...
            if(i == clist->num-1){
                int stdout =dup2(cli_sock, STDOUT_FILENO);
                if(stdout == -1){
                    perror("stdout failed: ");
                    exit(-1);                
                }
                int stderr = dup2(cli_sock, STDERR_FILENO);
                if(stderr == -1){
                    perror("stderr failed: ");
                    exit(-1);                
                }
            }
            if (i > 0) {
                int dupOne = dup2(pipes[i-1][0], STDIN_FILENO);
                if(dupOne == -1){
                    perror("");
                    exit(-1);
                }
            ...
            if (i < clist->num - 1) {
                int dupTwo = dup2(pipes[i][1], STDOUT_FILENO);
                if(dupTwo == -1){
                    perror("");
                    exit(-1);
                }
